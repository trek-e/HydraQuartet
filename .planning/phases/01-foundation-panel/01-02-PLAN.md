...
must_haves:
  ...
  artifacts:
    - path: "src/TriaxVCO.cpp"
      ...
      exports: ["TriaxVCO", "TriaxVCOWidget"]
      min_lines: 80
    - path: "plugin.cpp"
    ...
    - path: "plugin.json"
      ...
      contains: "TriaxVCO"
  key_links:
    - from: "src/TriaxVCO.cpp"
      to: "res/TriaxVCO.svg"
      via: "setPanel() in widget"
      pattern: 'setPanel.*TriaxVCO'
    - from: "src/TriaxVCO.cpp"
      to: "plugin.cpp"
      via: "Model registration"
      pattern: "createModel.*TriaxVCO"
    - from: "src/TriaxVCO.cpp"
      to: "dsp::FREQ_C4"
...
<objective>
Implement the TriaxVCO module with polyphonic infrastructure: 8-voice V/Oct tracking and audio output.
...
Output:
- Working TriaxVCO module that loads in VCV Rack
...
```bash
$RACK_DIR/helper.py createmodule TriaxVCO res/TriaxVCO.svg src/TriaxVCO.cpp
```
...
- Colors are exact: #ff0000, #00ff00, #0000ff
  ...
  <verify>
- `ls src/TriaxVCO.cpp` shows file exists
...
**In src/TriaxVCO.cpp, define the model:**
```cpp
...
- `grep -q "TriaxVCO" plugin.json` returns success
...
2. Launch VCV Rack and add Triax VCO module (search "Triax" in module browser)
